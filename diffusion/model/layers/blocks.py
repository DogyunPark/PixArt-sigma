...
#from openvid.acceleration.communications import all_to_all, split_forward_gather_backward
...
class MaskedMultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        attn_bias = None
        if mask is not None:
            attn_bias = torch.zeros((B, self.num_heads, S, 304)).to(x.device, torch.float16)
