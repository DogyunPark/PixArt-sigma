...
class FluxPipelineI2V(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        if self.do_classifier_free_guidance:
            ...
            prompt_embeds = torch.cat([uncond_prompt_embeds, prompt_embeds])
            prompt_embeds_mask = torch.cat([prompt_embeds_mask, prompt_embeds_mask])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #     noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond_textimage - noise_pred_cond_image) + 3 * (noise_pred_cond_image - noise_pred_uncond)
