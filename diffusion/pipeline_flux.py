...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def _get_t5_prompt_embeds(
        ...
        prompt_embeds = self.text_encoder_2(text_input_ids.to(device), attention_mask=txt_tokens.attention_mask)[0][:, None]
        prompt_embeds_mask = txt_tokens.attention_mask[:, None, None]
...
class FluxPipelineI2V_original(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    r"""
    The Flux pipeline for text-to-image generation.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond_textimage - noise_pred_cond_image) + 1.5 * (noise_pred_cond_image - noise_pred_uncond)
...
class FluxPipelineI2V(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    r"""
    The Flux pipeline for text-to-image generation.
