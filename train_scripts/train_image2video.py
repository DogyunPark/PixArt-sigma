...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step in range(num_steps_per_epoch):
            ...
            y = batch["text"]
            save_sample(x[0], fps=8, save_path=os.path.join('/data/cvpr25/test/', 'train_video.mp4'))
            import pdb; pdb.set_trace()
            if step < skip_step:
                global_step += 1
...
if __name__ == '__main__':
    ...
    if config.visualize:
        ...
        filename_list, data_list, validation_prompts = load_data_prompts(config.promptdir, video_size=config.image_size, video_frames=config.num_frames)
        #import pdb; pdb.set_trace()
        skip = True
